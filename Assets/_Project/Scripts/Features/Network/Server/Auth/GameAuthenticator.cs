...
namespace _Project.Scripts.Features.Network.Server.Auth
{
    public class GameAuthenticator : NetworkAuthenticator, IRaceRewardService
    {
        ...
        public void GrantCoinsToPlayer(NetworkConnectionToClient conn, int amount)
        {
            if (_authenticatedUsers.TryGetValue(conn.connectionId, out var email))
            {
                var user = _db.GetUserByEmail(email);
                if (user != null)
                {
                    ...
                    _db.UpdateCoins(email, newCoins);
                    Debug.Log($"[Server] Игрок {email} получил {amount} монет за гонку. Баланс: {newCoins}");
                }
            }
            else
            {
                Debug.LogWarning($"[Server] Попытка выдать монеты неавторизованному соединению {conn.connectionId}");
            }
        }
    }
}